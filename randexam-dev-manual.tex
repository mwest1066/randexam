...
$\sigma(n)=0$ for $n>1$ will allow no partial credit). The arrays
in the table above are computed by:
\begin{align}
  P_{\rm seq}(s,e,q) &= \sum_{a=1}^{N_{\rm A}} P\Big(Q(e,q), V(e,q), A(e,q,a)\Big)
...
\begin{align}
  ...
  n_{\rm QVC}(Q,V,C) &= \sum_{s=1}^{N_{\rm s}} \delta\left(C - \sum_{a=1}^{N_{\rm A}} b(s,q_{\rm sQ}(s,Q),a)\right)
  \delta\left(V-V_{\rm sQ}(s,Q)\right) \displaybreak[0]\\
  ...
  r^{\rm P}_{\rm Q}(Q) &= \frac{\sum_{s=1}^{N_{\rm s}}
    ...
    \sqrt{\sum_{s=1}^{N_{\rm s}} (P_{sQ}(s,Q) - \bar{P}_Q(Q))^2}
    \sqrt{\sum_{s=1}^{N_{\rm s}} (P_{s}(s) - \bar{P})^2}} \displaybreak[0] \\
