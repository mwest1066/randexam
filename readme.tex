...
\begin{center}
  \begin{tabular}{lll}
    ...
    $r^{\rm a}_{\rm ss}(s,\hat{s})$ & $N_{\rm s} \times N_{\rm s}$
    & proportion of identical exam answers for students $s$ and $\hat{s}$
  \end{tabular}
\end{center}
These quantities are computed by:
\begin{align}
  ...
  r^{\rm a}_{\rm ss}(s,\hat{s}) &= \frac{1}{N_Q} \sum_{q = 1}^{N_{\rm Q}}
...
$r^{\rm a}_{\rm ss}(s,\hat{s}) \approx 1/N_{\rm A}$ for $e(s) \ne
e(\hat{s})$, indicating only random association between the exam
answers given by students with distinct exams.
