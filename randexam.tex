...
text may only go into the locations indicated in the file.
...
As shown in Figure~\ref{fig:pipeline}, the processing pipeline begins
with the \texttt{library.tex} file. This file should be a valid
\LaTeX{} file that can be typeset and edited to prepare the
questions. To generate the randomized exams with the \texttt{randexam}
script, the format of \texttt{library.tex} must conform to the grammar
shown in Figure~\ref{fig:library-grammar}. The basic structure of
\texttt{library.tex} is thus a tree, as shown in
...
\end{grammar}
\end{minipage}
\caption{Grammer for the \texttt{library.tex} input file in partial
  Backus-Naur Form. The \textit{text} entries refer to any valid
  \LaTeX{} text, and \textit{points} is a numeric value.}
...
\begin{figure}
  ...
  \caption{State transition graph used by the \texttt{randexam} script
    to read the \texttt{library.tex} input file.}
